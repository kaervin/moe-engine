// retarded hashtable, but why change what works
...
struct table_entry {
	unsigned int key0, value0, key1, value1;
};
...
unsigned int crc32_cstring(const char *str);

Map init_map();

unsigned int map_lookup_key(Map *m, bool *ok, unsigned int key);
...
unsigned int crc32_cstring(const char *str) {
	...
	for(int i = 0; str[i] != 0; i++) {
		unsigned char lookupindex = (crc32 ^ str[i]) & 0xFF;
...
bool map_insert_kv(Map *m, unsigned int key, unsigned int value) {
	...
	if (te->value1 == 0) {
		fprintf( stderr, "key %u with value %u had 1 collision\n", key, value);
		te->key1 = key;
		te->value1 = value;
		return 1;
	}
